...
class Solution:
    def luckyNumbers (self, matrix: List[List[int]]) -> List[int]:
        m = len(matrix)
        n = len(matrix[0])
        matrixT = [[matrix[j][i] for j in range(m)] for i in range(n)]
        row_min = [row.index(min(row)) for row in matrix]
        col_max = [col.index(max(col)) for col in matrixT]
        res = []
        for i in range(m):
            minidx = row_min[i]
            if col_max[minidx] == i:
                res.append(matrix[i][minidx])
        return res
    def balanceBST(self, root: TreeNode) -> TreeNode:
        def lmr(node:TreeNode):
            if node is None:
                return []
            l = lmr(node.left)
            m = [node.val]
            r = lmr(node.right)
            return l + m + r
        def build_bst(arr:List):
            n = len(arr)
            if n == 0:
                return None
            if n == 1:
                return TreeNode(arr[0])
            mid = TreeNode(arr[n//2])
            mid.left = build_bst(arr[:n//2])
            mid.right = build_bst(arr[n//2+1:])
            return mid

        orderlist = lmr(root)
        bst = build_bst(orderlist)
        return bst
        ...
        #     if node is None:
        #         return 0,0,node
        #     lh,bfl,nl = dfs(node.left)
        #     node.left = nl
        #     rh,bfr,nr = dfs(node.right)
        #     node.right = nr
        #     bf = lh - rh
        #     if bf == 2:
        #         if bfl > 0:
        #             return max(lh,rh)+1, bf, LL(node)
        #         elif bfl < 0:
        #             return max(lh,rh)+1, bf, LR(node)
        #     elif bf == -2:
        #         if bfl < 0:
        #             return max(lh,rh)+1, bf, RR(node)
        #         elif bfl > 0:
        #             return max(lh,rh)+1, bf, RL(node)
        #     return max(lh,rh)+1, bf, node
        #
        # _,_,ret = dfs(root)
        # return ret
    ...
    def maxPerformance(self, n: int, speed: List[int], efficiency: List[int], k: int) -> int:
        ...
        for s,e in eng:
            ...
            tol_speed += s
            if group.qsize() > k:
                tol_speed -= group.get_nowait()

            best_score = max(best_score, tol_speed * e)

        return best_score % (10**9 + 7)

        # 0-1 背包 dfs
        # ret = [0]
        # def dfs(deep,nowk,total_speed,min_eff):
        #     if deep == n or nowk == 0:
        #         ret[0] = max(ret[0],total_speed*min_eff)
        #     dfs(deep+1,nowk,total_speed,min_eff)
        #     dfs(deep+1,nowk-1,total_speed+speed[deep],min(min_eff,efficiency[deep]))
        # dfs(0,k,0,10**8)
        # return ret[0] % (10**9 + 7)
