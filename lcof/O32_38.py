...
class Solution:
        ...
        def check(l,r):
            ...
            if div > -1:
                for i in range(div,r-1):
                    if postorder[i] < pivot:
                        return False
                return check(l,div) and check(div,r-1)
            else:
                return check(l,r-1)

        return check(0,len(postorder))
    ...
    def copyRandomList(self, head: 'Node') -> 'Node':
        ...
        while node:
            if node.random:
                cpynode.random = node_map[node.random]
            node = node.next
...
class Codec:
    ...
    def deserialize(self, data):
        ...
        """
        data = data[1:-1].split(',')
        data = list(map(lambda x: None if x == 'null' else int(x), data))
        n = len(data)
        if data[0] is None:
            return None
        queue = collections.deque()
        ...
        while queue and i < n:
            ...
            if data[i] is not None:
                q.left = TreeNode(data[i])
                queue.append(q.left)
            i+=1
            if data[i] is not None:
                q.right = TreeNode(data[i])
                queue.append(q.right)
            i+=1
        return root
    ...
    def permutation(self, s: str) -> List[str]:
        n = len(s)
        s = [[k,v] for k,v in collections.Counter(s).items()]
        m = len(s)
        ans = []
        def dfs(depth,a):
            if depth == n:
